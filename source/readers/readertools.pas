...
type
  TBookNote = record
    Id: string;        // идентификатор сноски (например, "note_1")
    Title: string;     // заголовок сноски (обычно "[1]")
    Text: string;      // текст сноски
  end;
  TBookNoteArray = array of TBookNote;
...
procedure ExtractTextContent(var CurPos: Integer;Content: String; var Output: string);
...
function IsContentTag(const Tag: string): Boolean;
function GetAttrValue(const TagStr, AttrName: string): string;
function InvertColor(const Color: TColorB): TColorB;
function InvertColorFull(const Color: TColorB): TColorB;
...
procedure ExtractTextContent(var CurPos: Integer; Content: String; var Output: string);
var
  StartPos, TagEnd: Integer;
begin
  StartPos := CurPos;
  while (CurPos <= Length(Content)) and (Content[CurPos] <> '<') do Inc(CurPos);
  if CurPos > StartPos then
    Output := Output + DecodeXMLEntities(Copy(Content, StartPos, CurPos - StartPos));

  if (CurPos <= Length(Content)) and (Content[CurPos] = '<') then
  begin
    TagEnd := Pos('>', Content, CurPos);
    if TagEnd > 0 then CurPos := TagEnd + 1;
  end;
end;
...
function IsContentTag(const Tag: string): Boolean;
begin
  Result := (CompareText(Tag, 'p') = 0) or
            (CompareText(Tag, 'text-author') = 0) or
            (CompareText(Tag, 'date') = 0) or
            (CompareText(Tag, 'strong') = 0) or
            (CompareText(Tag, 'em') = 0) or
            (CompareText(Tag, 'a') = 0) or
            (CompareText(Tag, 'strikethrough') = 0) or
            (CompareText(Tag, 'sub') = 0) or
            (CompareText(Tag, 'sup') = 0) or
            (CompareText(Tag, 'emphasis') = 0);
end;
...
begin
  Result := '';
  p := Pos(AttrName + '=', TagStr);
  if p = 0 then Exit;
  Inc(p, Length(AttrName) + 1);
  while (p <= Length(TagStr)) and (TagStr[p] in [' ', #9]) do Inc(p);
  if (p > Length(TagStr)) or not (TagStr[p] in ['"', '''']) then Exit;
  Quote := TagStr[p];
  Inc(p);
  q := Pos(Quote, TagStr, p);
  if (q > p) and (q <= Length(TagStr)) then
    Result := Copy(TagStr, p, q - p);
end;
...
begin
  ...
  Result.a := Color.a;  // Альфа-канал не инвертируем, оставляем как есть
end;
