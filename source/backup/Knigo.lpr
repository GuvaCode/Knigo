...
{$IFDEF LINUX} cthreads,{$ENDIF}
 ...
 Fb2Reader, ReaderTools, EpubToFb2Converter,MobiToFb2Converter, RtfToFb2Converter,
  gui.booknotes, gui.bookextra, gui.bookchapters, gui.booksettings; // readers
...
var MyIcon: TImage;
   ...
   TempFb2File, FCurrentBookFile: string;
begin
  ...
  // create the readers
   ...
   begin
     WriteLn('Обнаружен EPUB. Конвертирую в FB2...');
     TempFb2File := ChangeFileExt(FCurrentBookFile, '.fb2');

     if ConvertEpubToFb2(FCurrentBookFile, TempFb2File) then
     begin
       WriteLn('Конвертация успешна! Файл: ', TempFb2File);
       FCurrentBookFile := TempFb2File; // Переключаемся на сконвертированный файл
     end
     else
     begin
       WriteLn('Ошибка конвертации EPUB!');
       Halt(1);
     end;
   end;

   if LowerCase(ExtractFileExt(FCurrentBookFile)) = '.mobi' then
   begin
     WriteLn('Обнаружен Mobi. Конвертирую в FB2...');
     TempFb2File := ChangeFileExt(FCurrentBookFile, '.fb2');

     if ConvertMobiToFb2(FCurrentBookFile, TempFb2File) then
     begin
       WriteLn('Конвертация успешна! Файл: ', TempFb2File);
       FCurrentBookFile := TempFb2File; // Переключаемся на сконвертированный файл
     end
     ...
     begin
       WriteLn('Ошибка конвертации mobi!');
       Halt(1);
     end;
   end;

   if not FReader.LoadBook(FCurrentBookFile) then
   begin
