...
// === Эвристика для распознавания заголовков глав ===
function IsLikelyChapterTitle(const Text: string): Boolean;
var
  Clean: string;
  i: Integer;
  IsRoman: Boolean;
begin
  Clean := Trim(Text);
  if Clean = '' then Exit(False);

  // ИСПРАВЛЕНИЕ 1: Заголовки глав обычно короткие.
  // Если текст слишком длинный, это не заголовок (например, встроенное оглавление).
  if Length(Clean) > 120 then Exit(False);

  // 1. Проверка на римские цифры
  IsRoman := True;
  for i := 1 to Length(Clean) do
    if not (Clean[i] in ['I', 'V', 'X', 'L', 'C', 'D', 'M', ' ', '.', ',']) then
    begin
      IsRoman := False;
      Break;
    end;
  if IsRoman and (Length(Trim(Clean)) > 0) and (Length(Trim(Clean)) <= 15) then
    Exit(True);

  // 2. Проверка на ключевые слова
  Clean := LowerCase(Clean);
  if (Pos('глава', Clean) = 1) or (Pos('chapter', Clean) = 1) or
     (Pos('часть', Clean) = 1) or (Pos('part', Clean) = 1) or
     (Pos('prologue', Clean) = 1) or (Pos('epilogue', Clean) = 1) or
     (Pos('предисловие', Clean) = 1) or (Pos('послесловие', Clean) = 1) or
     (Pos('введение', Clean) = 1) then
    Exit(True);
...
begin
  Result := '';
  AttrStart := Pos(AttrName + '=', Tag);
  if AttrStart = 0 then Exit;
  ValStart := AttrStart + Length(AttrName) + 1;
...
begin
  ...
  Result := string(PAnsiChar(@Data[0]));
  ConvertEncodingIfNeeded(Result);
end;
...
var
  ...
  AlreadyHasTitle: Boolean; // Новая переменная для проверки дублей
    ...
    try
      ...
      begin
        ...
        for ElemIdx := 0 to High(Elements) do
          if Elements[ElemIdx].ElemType in [cetParagraph, cetTitle, cetSubtitle, cetVerse, cetEmphasis, cetStrong, cetBlockquote] then
          begin
            HasContent := True;
            Break;
          end;
        ...
        // ИСПРАВЛЕНИЕ 2: Защита от дублирования заголовков
        if ExplicitChapterTitle <> '' then
        begin
          AlreadyHasTitle := False;
          for ElemIdx := 0 to High(Elements) do
          begin
            // Если в XHTML уже есть заголовок, который совпадает с тем, что в toc.ncx, не дублируем
            ...
            // Если дошли до основного текста, значит заголовка не было
        ...
        begin
          ...
          begin
            if IsLikelyChapterTitle(Elem.Text) then
              ActualType := cetTitle
            else
      ...
      begin
        ...
        WriteString(ImagesList[ImgIdx].Base64Data + #10);
