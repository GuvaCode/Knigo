...
var
  ...
  // Надежная функция схлопывания множественных пробелов и переносов строк в один пробел
...
begin
  ...
  begin
    ...
    begin
      ...
      // Заменяем HTML-сущность неразрывного пробела на обычный пробел
      TextContent := StringReplace(TextContent, '&nbsp;', ' ', [rfReplaceAll, rfIgnoreCase]);

      // Надежное схлопывание всех пробельных символов (пробелы, табы, переносы строк) в один пробел
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if (ActualType <> cetTitle) and (not SectionOpen) then
          begin
